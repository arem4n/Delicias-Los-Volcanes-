...
export const AdminPanel: React.FC<Props> = ({ isOpen, onClose }) => {
  ...
  const handleSaveProduct = async (e: React.FormEvent) => {
    ...
    try {
      const result = await callApi('saveProduct', { product: editingProduct });
      if (editingProduct.id) {
        setProducts(prev => prev.map(p => p.id === editingProduct.id ? editingProduct : p));
      } else {
        const newProductWithId = { ...editingProduct, id: result.newId };
  ...
  const handleDeleteProduct = async (id: string) => {
    if (confirm('¿Eliminar este producto? Esta acción no se puede deshacer.')) {
      try {
        await callApi('deleteProduct', { productId: id });
        ...
        <div className="flex-1 overflow-y-auto p-8">
          {isLoading ? (
            <div className="h-full flex items-center justify-center">
              <Loader className="animate-spin text-brand-gold" size={48} />
            </div>
          ) : error ? (
                    ...
                    <div className="flex flex-col md:flex-row justify-between gap-6">
                      <div className="space-y-3">
                        ...
                        <h4 className="text-xl font-bold text-brand-brown">
                          ...
                          <span className="text-sm font-light text-stone-400 ml-2">({order.cliente_email})</span>
                        </h4>
                        <div className="text-sm text-stone-500">
                          {JSON.parse(order.detalle_json).map(item => `${item.quantity}x ${item.nombre}`).join(', ')}
                        </div>
                      </div>
